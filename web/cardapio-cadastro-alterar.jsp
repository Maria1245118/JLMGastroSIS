<%-- 
    Document   : cardapio-cadastro-alterar
    Created on : 8 de jan de 2021, 08:14:29
    Author     : Janine
--%>
...
<jsp:include page = "header.jsp">
    <jsp:param name="tituloPagina" value="Alteração de Dados no Cadastro de Cardápios" />
</jsp:include>
<%
...
%>
                <h1>Alteração de Dados no Cadastro de Cardápios</h1>
                <form action="cardapio-recebe-cadastro-alterar.jsp" method="POST" onsubmit="return enviarForm()">
                    <input type="hidden" name="qtdePratos" id="qtdePratos" value="<%=menu.getPratos().size()%>"/>
                    <p class="txt-verde txt-center p-block">Campos com Preenchimento Obrigatório <span class="asterisco">*</span></p>
                    
                    <div class="form-grupo">
                        <label>Código</label>
                        <input  class="form-campo form-campo-w250" type="text" name="idcardapio" readonly="true" value="<%out.write(String.valueOf(menu.getIdCardapio()));%>"/>
                    </div>
                    <div class="form-grupo">
                        <label>Nome do Cardápio <span class="asterisco">*</span></label>
                        <input  class="form-campo" type="text" name="desccardapio" value="<%out.write(menu.getDescCardapio());%>"/>
                        <div class="alerta alerta-vermelho d-none" id="erroDesc"></div>
                    </div>
                    <div class="form-grupo">
                        <label>Nº de Pessoas <span class="asterisco">*</span></label>
                        <input class="form-campo form-campo-w250 txt-right" type="text" name="qtdepessoas" id="qtdepessoas" onchange="calculaUnitario()"  value="<%out.write(Web.inteiroParaString(menu.getQtdePessoas()));%>"/>
                        <div class="alerta alerta-vermelho d-none" id="erroQtde"></div>
                    </div>
                    <div class="form-grupo">
                        <label>Preço por Pessoa</label>
                        <input class="form-campo form-campo-w250 txt-right" type="text" readonly="true" name="valorunitario" id="valorunitario"  value="<%=Web.moedaParaString(menu.getValorUnitario())%>"/>
                    </div>
                    <div class="form-grupo">
                        <label>Preço do Cardápio</label>
                        <input class="form-campo form-campo-w250 txt-right" type="text" readonly="true" name="valortotal" id="valortotal"  value="<%=Web.moedaParaString(menu.getValorTotal())%>"/>
                    </div>
                    <div class="form-checkbox">
                        <input type="checkbox" name="buffetinterno" id="buffetinterno"  <%out.write(menu.isBuffetInterno() ? " checked='checked'" : "");%>/> 
                                ...
                                <tbody>
                                    <%
                                        ...
                                        for (PratoCardapio prmenu : menu.getPratos()) {
                                            prato = prato.consultar(prmenu.getIdPrato());
                                    %>
                                    <tr class="selection">
                                        <td> <!--Prato-->
                                            <select  class="form-campo" id="idprato<%=i%>" name="idprato<%=i%>" onchange="extraiPreco(this)">
                                                <option value=""></option>
                                                <%for (Prato pr : pratos) {%>
                                                    <option value="<%out.write(String.valueOf(pr.getIdPrato()));%>" 
                                                        <%out.write(prmenu.getIdPrato()==pr.getIdPrato() ? " selected='selected'" : "");%>>
                                                        <%out.write(pr.getNome() + " - Rende: " + pr.getRendimento() + " porções - " 
                                                         + Web.moedaParaString(pr.getValorPrato()));%>
                                                    </option>
                                                <% } %>
                                            </select>
                                            <div class="alerta alerta-vermelho d-none" id="erroPrato<%=i%>"></div>
                                        </td>
                                        <td><input class="form-campo txt-right" type="text" readonly="true" id="preco<%=i%>"  value="<%=Web.moedaParaString(prato.getValorPrato())%>"/></td>
                                    </tr>
                                    <%
                                        i++;
                                        }
                    ...
                    <div class="grupo-botoes">
                        ...
                        <button type="reset" class="btn btn-vermelho" onclick="document.location='cardapio-consultar.jsp';">Cancelar</button>
                    </div>
                </form>
        <script>
            function enviarForm() {
            ...
            function extraiPreco(listaPratos) {
                ...
                for (p = 1; p <= qtdePratos; p++) {
                    totalCardapio += parseFloat(document.getElementById('preco' + p).value.replace(',', '.').replace('R$','').trim());
                }
